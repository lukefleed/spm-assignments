...
\section*{Results}
This section presents a comparative analysis of three softmax function implementations under varying conditions. We evaluate the \texttt{softmax\_auto} implementation with and without the \texttt{parallel} directive and compare performance between AVX2 and AVX512 instruction sets. It is important to note that only the auto-vectorized implementation (\texttt{softmax\_auto}) adapts to use AVX512 instructions when available, while the manually vectorized implementation (\texttt{softmax\_avx}) uses explicit AVX2 instructions regardless of the compilation flags.
...
We compare the execution time of three softmax implementations across various input sizes, analyzing the effects of parallelization and vectorization instruction sets. The auto-vectorized implementation adapts its vectorization strategy based on available instruction sets (AVX512 when available, falling back to AVX2), while the manually vectorized implementation consistently uses AVX2 instructions. Figures~\ref{fig:perf_noparallel_avx512} and \ref{fig:perf_noparallel_noavx512} demonstrate performance without parallelization, while Figures~\ref{fig:perf_parallel_avx512} and \ref{fig:perf_parallel_noavx512} show results with parallelization enabled.
