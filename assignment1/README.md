...
The benchmark framework distinguishes between AVX512-enabled and non-AVX512 configurations:

- **Auto-vectorized implementation** (`softmax_auto`): Automatically adapts to use AVX512 instructions when available and compiled with appropriate flags. On processors lacking AVX512 support, it falls back to AVX2 or other available instruction sets.
- **Manual AVX implementation** (`softmax_avx`): Uses explicit AVX2 instruction sets regardless of AVX512 compilation flags or hardware availability. This ensures consistent behavior but limits performance on AVX512-capable processors.

It is important to note that on processors lacking AVX2 support, the manually vectorized implementation will not work.
